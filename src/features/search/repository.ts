...
export async function searchDocuments(
  ...
  return prisma.$queryRaw<SearchDocumentItem[]>`
    ...
    exact_matches AS (
      ...
      WHERE d."ownerId" = ${ownerId}
        ...
        AND d.title ILIKE search_input.query
    ...
    prefix_matches AS (
      ...
      WHERE d."ownerId" = ${ownerId}
        ...
        AND d.title ILIKE search_input.query || '%'
      ORDER BY
        CASE WHEN d.title ILIKE search_input.query THEN 1 ELSE 0 END DESC,
        d."updatedAt" DESC
      LIMIT ${candidateLimit}
    ),
    ...
    trigram_matches AS (
      SELECT
        ...
        0::int AS is_prefix,
        0::float8 AS fts_rank,
        similarity(d.title, search_input.query)::float8 AS trigram_similarity,
        NULL::text AS snippet
      FROM "Document" d
      CROSS JOIN search_input
      WHERE d."ownerId" = ${ownerId}
        AND d."archivedAt" IS NULL
        AND d.title % search_input.query
      ORDER BY
        similarity(d.title, search_input.query) DESC,
        d."updatedAt" DESC
      LIMIT ${candidateLimit}
    ),
